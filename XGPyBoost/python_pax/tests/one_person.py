...
def main():
    ...
    TARGET_MODEL_NAME = "target_modelDebugtexasonepersonxgb.pkl"
    ...
    is_he_in = attack_model.predict(target_model.predict_proba(np.array([person])))
    person[10] = 0.02492735892338278
    is_other_in = attack_model.predict(target_model.predict_proba(np.array([person])))
    person[10] = 0.680073405719529
    print(f"another {attack_model.predict(target_model.predict_proba(np.array([person])))}")
